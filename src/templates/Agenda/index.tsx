...
const models = ["vmw", "VMW", "cat", "audi", "Toyota", "Long pink cat", "mercedes"]
const words = ["today", "is", "a", "good", "day", "to", "be", "alive", ".", "Try", "not", "to", "copy", "me"]
const titles = ["fre", "Pastilles Fre", "manega", "roda davant", "mirall", "mirallets delanters", "pneumatic", "iguana"]

function pickRand<T>(list: T[]): T {
	return list[Math.trunc(Math.random() * list.length)]
}

function genOrder(date) {
	let title = pickRand(titles)
	return {
		description: title + " " + Array(randInt(100)).fill(0).map(_ => pickRand(names)).join(" "),
		title,
