...
export function bindKey(element: Listener, keymap: KeyMap, listener: () => "propagate" | void | Promise<void>) {
	const onKeyDown = (event: KeyboardEvent) => {
		...
		if (event.key == key || event.code == key || event.code == "Key" + key || event.code == "Digit" + key) {
